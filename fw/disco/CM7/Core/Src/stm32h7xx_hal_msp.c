...
void HAL_QSPI_MspInit(QSPI_HandleTypeDef* hqspi)
{
  ...
  */
    ...
    */
    ...
    /* CubeMX defaults all QSPI pins to LOW slew which is far too slow for
     * the ~80 MHz QSPI clock (HCLK/3). Re-init every QSPI pin at
     * VERY_HIGH speed. Additionally pull IO2 (WP#) and IO3 (HOLD#) high
     * during 1-line command phases so the chip is not held in HOLD/WP
     * state — the controller does not drive those lines in 1-line mode.
     * (Place this in the USER CODE block so it survives CubeMX regen.) */
    {
        GPIO_InitTypeDef qspi_pin = {0};
        qspi_pin.Mode  = GPIO_MODE_AF_PP;
        qspi_pin.Pull  = GPIO_NOPULL;
        qspi_pin.Speed = GPIO_SPEED_FREQ_VERY_HIGH;

        qspi_pin.Alternate = GPIO_AF9_QUADSPI;
        qspi_pin.Pin = QSPI_BK2_IO2_Pin | QSPI_BK2_IO3_Pin;
        HAL_GPIO_Init(GPIOG, &qspi_pin);
        qspi_pin.Pin = QSPI_BK2_IO0_Pin | QSPI_BK2_IO1_Pin;
        HAL_GPIO_Init(GPIOH, &qspi_pin);
        qspi_pin.Pin = QSPI_CLK_Pin;
        HAL_GPIO_Init(QSPI_CLK_GPIO_Port, &qspi_pin);
        qspi_pin.Pin = QSPI_BK1_IO0_Pin;
        HAL_GPIO_Init(QSPI_BK1_IO0_GPIO_Port, &qspi_pin);

        qspi_pin.Alternate = GPIO_AF10_QUADSPI;
        qspi_pin.Pin = QSPI_BK1_NCS_Pin;
        HAL_GPIO_Init(QSPI_BK1_NCS_GPIO_Port, &qspi_pin);
        qspi_pin.Pin = QSPI_BK1_IO1_Pin;
        HAL_GPIO_Init(QSPI_BK1_IO1_GPIO_Port, &qspi_pin);

        /* IO2/IO3 with internal pull-up (WP#/HOLD# inactive). */
        qspi_pin.Alternate = GPIO_AF9_QUADSPI;
        qspi_pin.Pull = GPIO_PULLUP;
        qspi_pin.Pin = QSPI_BK1_IO3_Pin | QSPI_BK1_IO2_Pin;
        HAL_GPIO_Init(GPIOF, &qspi_pin);
    }
